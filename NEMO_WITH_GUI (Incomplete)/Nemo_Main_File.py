...
def LISTEN(stop_listening, shared_dict,stop_flag):
    ...
    def process_audio(audio, stop_listening, shared_dict):
        ...
        try:
            ...
            if "nemo" in recognized_text.lower() or "imo" in recognized_text.lower() or "jarvis" in recognized_text.lower() or"nemo" in recognized_text.lower() :
                print("ACTIVATING SIR>>>>..........")
                try:
                    recognized_text =re.sub(r"\bnemo\b", "", recognized_text, flags=re.IGNORECASE).strip()
                    recognized_text =re.sub(r"\bjarvis\b", "", recognized_text, flags=re.IGNORECASE).strip()
